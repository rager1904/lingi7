...
class FeatureExtractor:
    ...
    PAYMENT_VELOCITY_WINDOW_MINUTES = 10
    ORDER_VELOCITY_WINDOW_MINUTES = 60
    ...
    def build_rule_context(cls, order_id: int) -> RuleContext:
        ...
        payment_window = now - timedelta(minutes=cls.PAYMENT_VELOCITY_WINDOW_MINUTES)
        ...
        order_window = now - timedelta(minutes=cls.ORDER_VELOCITY_WINDOW_MINUTES)
        ...
        first_payment = PaymentAttempt.objects.filter(user=buyer).order_by("created_at").first()
        if first_payment:
