...
class VendorShipmentViewSet(viewsets.ViewSet):
    ...
    def _get_vendor_order(self, request: Request, order_id: int):
        ...
        try:
            return Order.objects.get(pk=order_id, vendor=request.user)
    ...
    def retrieve(self, request: Request, pk: int) -> Response:
        ...
        try:
            shipment = (
                Shipment.objects
                .prefetch_related("events")
                .get(pk=pk, order__vendor=request.user)
            )
        except Shipment.DoesNotExist:
    ...
    def dispatch(self, request: Request, pk: int) -> Response:
        ...
        """
        try:
            shipment = Shipment.objects.get(pk=pk, order__vendor=request.user)
        except Shipment.DoesNotExist:
            raise NotFound("Shipment not found.")
