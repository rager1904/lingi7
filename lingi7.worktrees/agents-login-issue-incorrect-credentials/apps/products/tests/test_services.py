...
class TestStoreServiceApprove(TestCase):
    ...
    def test_approve_logs_to_audit(self):
        ...
        StoreService.approve_store(store=self.store, admin_user=self.admin)
        log = AdminAuditLog.objects.get(action="STORE_APPROVED", object_id=str(self.store.pk))
        self.assertEqual(log.before_state, Store.Status.PENDING)
        self.assertEqual(log.after_state, Store.Status.APPROVED)
