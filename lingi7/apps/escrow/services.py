...
class EscrowService:
    ...
    def hold_funds(
        *,
        ...
        account.version += 1
        account.save(update_fields=["balance", "state", "version", "updated_at"])
    ...
    def mark_in_transit(
        *,
        ...
        account.version += 1
        account.save(update_fields=["state", "version", "updated_at"])
    ...
    def release_funds(
        *,
        ...
        if should_freeze:
            ...
            account.version += 1
            account.save(update_fields=["state", "frozen_at", "version", "updated_at"])
            EscrowService._log_audit(
                actor_ref=actor_ref,
        ...
        account.version += 1
        account.save(update_fields=["balance", "state", "released_at", "version", "updated_at"])
    ...
    # ------------------------------------------------------------------
    # Partial refund (DISPUTED / HELD → PARTIALLY_REFUNDED)
    # ------------------------------------------------------------------
    ...
    def partial_refund(
        *,
        ...
        """
        Refund a portion of held funds to the buyer, retaining the rest.

        Used for split-resolution disputes where the buyer gets a partial
        refund. The remaining balance stays in escrow for potential
        release to the vendor or further dispute resolution.

        Args:
            account_id: PK of the EscrowAccount.
            amount: ZMW amount to refund (must be > 0 and <= balance).
            actor_ref: UUID of the admin resolving the dispute.
            reason: Refund reason for the audit trail.

        Returns:
            Updated EscrowAccount in PARTIALLY_REFUNDED state.
        ...
        """
        if amount <= Decimal("0.00"):
            raise ValueError(f"partial_refund: amount must be positive, got {amount}")

        account = EscrowAccount.objects.select_for_update().get(pk=account_id)

        if amount > account.balance:
            raise InsufficientBalanceError(
                f"Partial refund amount ZMW {amount} exceeds "
                f"account balance ZMW {account.balance}."
            )

        valid_sources = {EscrowState.HELD, EscrowState.DISPUTED, EscrowState.PARTIALLY_REFUNDED}
        if account.state not in valid_sources:
            raise InvalidTransitionError(account.state, EscrowState.PARTIALLY_REFUNDED)
        ...
        EscrowService._write_ledger_pair(
            account=account,
            amount=amount,
            debit_description=f"Partial refund to buyer — {reason}",
            credit_description=f"Buyer partial refund reversal — {reason}",
            operation_ref=operation_ref,
            actor_ref=actor_ref,
        ...
        account.state = EscrowState.PARTIALLY_REFUNDED
        account.version += 1
        account.save(update_fields=["balance", "state", "version", "updated_at"])

        EscrowService._log_audit(
            actor_ref=actor_ref,
            action="ESCROW_PARTIAL_REFUND",
            object_id=account.id,
            before_state=before_state,
            after_state=EscrowState.PARTIALLY_REFUNDED,
            notes=f"amount=ZMW {amount}, remaining=ZMW {new_balance}, reason={reason}",
        )
        logger.info(
            "Partial refund: account=%s amount=ZMW %s remaining=ZMW %s",
        ...
        *,
        ...
        try:
            ...
            AdminAuditLog.objects.create(
                ...
                target_repr=f"EscrowAccount({object_id})",
