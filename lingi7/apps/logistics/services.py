...
class LogisticsService:
    ...
    def create_shipment(
        ...
        **kwargs: Any,
        ...
        AdminAuditLog.objects.create(
            ...
            target_content_type="logistics.shipment",
            target_object_id=str(shipment.pk),
            target_repr=f"Shipment({shipment.pk})",
            before_state=None,
            after_state={"status": Shipment.Status.CREATED},
        )
    ...
    def transition_status(
        ...
        AdminAuditLog.objects.create(
            actor=actor,
            action_type=f"SHIPMENT_STATUS_{to_status}",
            target_content_type="logistics.shipment",
            target_object_id=str(shipment.pk),
            target_repr=f"Shipment({shipment.pk})",
    ...
    def confirm_delivery(
        ...
        updated_shipment = LogisticsService.transition_status(
            ...
            source=(
                ...
                else TrackingEvent.Source.VENDOR_MANUAL
                if confirmed_by in ("BUYER", "SELLER")
                else TrackingEvent.Source.CARRIER_WEBHOOK
                if confirmed_by == "CARRIER"
                else TrackingEvent.Source.ADMIN_MANUAL
                if confirmed_by == "ADMIN"
                else TrackingEvent.Source.SYSTEM
            ),
        )
    ...
    def update_carrier_tracking_number(
        ...
        if actor:
            AdminAuditLog.objects.create(
                actor=actor,
                action_type="SHIPMENT_TRACKING_UPDATED",
                target_content_type="logistics.shipment",
                target_object_id=str(shipment.pk),
                target_repr=f"Shipment({shipment.pk})",
                before_state={"status": shipment.status},
                after_state={"status": shipment.status},
            )
    ...
    def _get_system_actor() -> Any:
        ...
        Raises a critical log if no superuser exists at all.
        """
        ...
        try:
            ...
            if actor is None:
                logger.critical("No superuser found — audit log will have null actor")
            return actor
