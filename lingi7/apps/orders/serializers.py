...
class OrderLineCreateSerializer(serializers.Serializer):
    """Server resolves price from catalog — clients send product_id + quantity only."""
...
class OrderSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            "escrow_status", "item_count",
        ]
        read_only_fields = fields

    def get_escrow_status(self, obj: Order) -> str:
        """Expose linked escrow state for the React order UI."""
        if obj.escrow_account_id and getattr(obj, "escrow_account", None):
            return obj.escrow_account.state
        return _ORDER_STATUS_ESCROW_FALLBACK.get(obj.status, "PENDING")

    def get_item_count(self, obj: Order) -> int:
        return sum(line.quantity for line in obj.lines.all())


_ORDER_STATUS_ESCROW_FALLBACK = {
    OrderStatus.DRAFT: "PENDING",
    OrderStatus.PENDING_PAYMENT: "PENDING",
    OrderStatus.PAYMENT_RECEIVED: "HELD",
    OrderStatus.PROCESSING: "HELD",
    OrderStatus.SHIPPED: "IN_TRANSIT",
    OrderStatus.DELIVERED: "DELIVERED",
    OrderStatus.COMPLETED: "RELEASED",
