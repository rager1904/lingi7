...
class TestCreateOrder:
    ...
    def test_unknown_product_raises(self, buyer, seller):
    ...
    def test_zero_quantity_raises(self, buyer, seller, catalog_products):
        p1, _ = catalog_products
        with pytest.raises(OrderServiceError, match=">= 1"):
            OrderService.create_order(
                buyer=buyer,
                seller=seller,
                lines=[{"product_id": p1.pk, "quantity": 0}],
            )
...
class TestSubmitOrder:
    def test_submit_transitions_to_pending(self, draft_order, buyer):
        ...
        assert order.escrow_account_id is not None
...
class TestConfirmPayment:
    def test_confirm_transitions_to_payment_received(self, pending_order, buyer):
        ...
        mock_attempt.amount = pending_order.total_amount
        mock_attempt.provider = "MTN_MOMO"
        mock_attempt.provider_reference = "REF-001"
    ...
    def test_escrow_hold_called(self, pending_order, buyer):
        ...
        mock_attempt.amount = pending_order.total_amount
        mock_attempt.provider = "MTN_MOMO"
        mock_attempt.provider_reference = "REF-002"
