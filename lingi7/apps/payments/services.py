...
class ProviderCircuitOpenError(PaymentError):
    """Raised when the provider circuit breaker is open."""


class ProviderCircuitBreaker:
    """
    Redis-backed circuit breaker for mobile money providers.

    After FAILURE_THRESHOLD consecutive failures, the circuit opens
    for RECOVERY_TIMEOUT seconds, during which all calls fail fast
    without hitting the provider API.
    """

    FAILURE_THRESHOLD = 5
    RECOVERY_TIMEOUT = 300  # 5 minutes
    _PREFIX = "payments:circuit"

    @classmethod
    def _state_key(cls, provider: str) -> str:
        return f"{cls._PREFIX}:{provider}"

    @classmethod
    def _failures_key(cls, provider: str) -> str:
        return f"{cls._PREFIX}:{provider}:failures"

    @classmethod
    def is_open(cls, provider: str) -> bool:
        from django.core.cache import cache
        return cache.get(cls._state_key(provider)) == "OPEN"

    @classmethod
    def record_success(cls, provider: str) -> None:
        from django.core.cache import cache
        cache.delete(cls._failures_key(provider))
        cache.delete(cls._state_key(provider))

    @classmethod
    def record_failure(cls, provider: str) -> None:
        from django.core.cache import cache
        key = cls._failures_key(provider)
        failures = cache.get(key, 0) + 1
        cache.set(key, failures, timeout=600)
        if failures >= cls.FAILURE_THRESHOLD:
            cache.set(cls._state_key(provider), "OPEN", timeout=cls.RECOVERY_TIMEOUT)
            logger.warning(
    ...
    @classmethod
    def check(cls, provider: str) -> None:
        """Raise ProviderCircuitOpenError if circuit is open."""
        if cls.is_open(provider):
            raise ProviderCircuitOpenError(
                f"Provider {provider} circuit is open. "
                f"Retry after {cls.RECOVERY_TIMEOUT}s."
            )
...
class PaymentService:
    ...
    def initiate_collection(
        *,
        ...
        payer_phone: str,
        reference: str,
        initiated_by_id: str | uuid.UUID | None = None,
    ) -> PaymentAttempt:
        """
        ...
        try:
            linked_order = Order.objects.get(escrow_account_id=escrow_account_id)
            if linked_order.status not in (
                OrderStatus.DRAFT,
                OrderStatus.PENDING_PAYMENT,
            ):
                raise PaymentError(
                    f"Order cannot accept payment in status {linked_order.status}."
                )
            if amount != linked_order.total_amount:
                raise PaymentError("Payment amount does not match order total.")
        except Order.DoesNotExist:
            pass

        # Check attempt count before proceeding (non-success attempts only)
        existing_attempts = PaymentAttempt.objects.filter(
            escrow_account_id=escrow_account_id,
            direction=PaymentAttempt.Direction.COLLECTION,
        ).exclude(status=PaymentAttempt.Status.SUCCESS).count()
        ...
        ProviderCircuitBreaker.check(provider)
        try:
        ...
        # For mock payments, schedule auto-completion (simulates USSD approval)
        if (
            result.get("success")
            and attempt.provider_reference
            and "[MOCK]" in (attempt.provider_response_body.get("message") or "")
        ):
            from .tasks import auto_complete_mock_payment
            auto_complete_mock_payment.apply_async(
                args=[str(attempt.id)],
                countdown=3,  # Auto-complete after 3 seconds
            )
            logger.info(
                "Scheduled mock payment auto-completion in 3s: attempt=%s",
                attempt.id,
            )
    ...
    def _call_collection_api(
        *,
        ...
        Falls back to mock provider in development if real provider fails.
        This allows testing payment flows without real credentials.
        """
        ...
        try:
            ...
            # In development/debug mode, fall back to mock provider
            if getattr(settings, "DEBUG", False):
                logger.warning(
                    "Real provider %s failed (%s), falling back to mock provider for testing",
                    provider,
                    exc,
                )
                from .providers.mock import MockPaymentProvider
                result = MockPaymentProvider.request_to_pay(
                    amount=amount,
                    payer_phone=payer_phone,
                    reference=reference,
                    message=f"[MOCK] Lingi7 payment {reference}",
                )
                return {
                    "success": result.success,
                    "provider_reference": result.provider_reference,
    ...
    def initiate_disbursement(
        *,
        ...
        # Amount tamper protection — verify against escrow hold record
        from apps.escrow.models import EscrowHold
        try:
            hold = EscrowHold.objects.get(account_id=escrow_account_id)
            if hold.net_amount > Decimal("0.00") and amount != hold.net_amount:
                raise PaymentError(
                    f"Disbursement amount ZMW {amount} != escrow net ZMW {hold.net_amount}."
                )
        except EscrowHold.DoesNotExist:
            pass  # Allow disbursement without hold record (edge case)
        ...
        *,
        ...
        if is_success and attempt:
            ...
            order = Order.objects.filter(
                escrow_account_id=attempt.escrow_account_id
            ).first()
            if order and attempt.amount != order.total_amount:
                logger.error(
                    "Webhook amount mismatch: attempt=%s order=%s",
                    attempt.amount,
                    order.total_amount,
                )
                webhook_event.status = WebhookEvent.Status.INVALID
                webhook_event.processing_error = "Payment amount mismatch"
                webhook_event.save(update_fields=["status", "processing_error"])
                return
