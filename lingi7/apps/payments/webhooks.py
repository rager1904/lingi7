...
from .models import Provider, WebhookDeadLetter
...
def _save_dead_letter(
    ...
    """Persist a failed webhook to the dead letter queue for manual retry."""
    try:
        # Find the original WebhookEvent by provider_reference
        ...
        logger.info("Dead letter created for ref=%s", provider_reference)
    except Exception:  # noqa: BLE001
        logger.exception("Failed to save dead letter for ref=%s", provider_reference)
...
def _validate_mtn_signature(request: HttpRequest) -> bool:
    ...
    if not expected_token:
        if settings.DEBUG:
            logger.warning(
                "MTN_MOMO_CALLBACK_TOKEN not configured — allowing webhook in DEBUG only"
            )
            return True
        logger.error("MTN_MOMO_CALLBACK_TOKEN not configured — rejecting webhook")
        return False
...
def _validate_airtel_signature(request: HttpRequest, raw_body: bytes) -> bool:
    ...
    if not client_secret or not received_sig:
        if settings.DEBUG and getattr(settings, "AIRTEL_MONEY_ENVIRONMENT", "sandbox") == "sandbox":
            logger.debug("Airtel sandbox DEBUG — skipping signature validation")
            return True
        logger.warning("Missing Airtel signature or client_secret — rejecting webhook")
        return False
...
class MTNMoMoWebhookView(View):
    ...
    def post(self, request: HttpRequest) -> HttpResponse:
        ...
        if not signature_valid:
            logger.warning(
                "MTN webhook rejected (invalid signature): ref=%s", provider_reference
            )
        ...
        try:
            ...
            logger.exception(
                "MTN webhook processing failed: ref=%s", provider_reference
            )
            _save_dead_letter(
                provider=Provider.MTN_MOMO,
                provider_reference=provider_reference,
        ...
        # Always return 200 on accepted payloads — MTN retries on 4xx/5xx
        return HttpResponse(status=200)
...
class AirtelMoneyWebhookView(View):
    ...
    def post(self, request: HttpRequest) -> HttpResponse:
        ...
        if not signature_valid:
            logger.warning(
                "Airtel webhook rejected (invalid signature): ref=%s", provider_reference
            )
            return HttpResponse(status=401)
        ...
        try:
            ...
            logger.exception(
                "Airtel webhook processing failed: ref=%s", provider_reference
            )
            _save_dead_letter(
                provider=Provider.AIRTEL,
                provider_reference=provider_reference,
