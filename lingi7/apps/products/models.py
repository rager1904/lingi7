...
class Product(models.Model):
    ...
    # Catalog enrichment (Phase 1 — Ollama / heuristic pipeline)
    class EnrichmentStatus(models.TextChoices):
        PENDING = "PENDING", "Not enriched"
        PROCESSING = "PROCESSING", "Enrichment in progress"
        COMPLETED = "COMPLETED", "Enrichment complete"
        FAILED = "FAILED", "Enrichment failed"
        DISABLED = "DISABLED", "Enrichment disabled"
    ...
    ai_enhanced_title = models.CharField(max_length=200, blank=True)
    ai_features = models.JSONField(default=list, blank=True)
    ai_specs = models.JSONField(default=dict, blank=True)
    ...
    image_quality_scores = models.JSONField(default=dict, blank=True)
    descriptions_i18n = models.JSONField(default=dict, blank=True)
