...
class PublicProductListSerializer(serializers.ModelSerializer):
    ...
    seller_id = serializers.UUIDField(source="store.owner_id", read_only=True)
    ...
    def get_stock_quantity(self, obj: Product) -> int:
        ...
        if inv is None or not inv.track_inventory:
            return 999
        return max(0, inv.quantity_available)

    def get_is_in_stock(self, obj: Product) -> bool:
        inv = getattr(obj, "inventory", None)
        if inv is None:
            return True
        return bool(inv.is_in_stock)
...
class PublicProductDetailSerializer(serializers.ModelSerializer):
    ...
    descriptions_i18n = serializers.SerializerMethodField()
    ...
    def _is_enriched(self, obj: Product) -> bool:
        return obj.enrichment_status == Product.EnrichmentStatus.COMPLETED
    ...
    def get_features(self, obj: Product) -> list:
        return obj.ai_features if self._is_enriched(obj) else []

    def get_specs(self, obj: Product) -> dict:
        return obj.ai_specs if self._is_enriched(obj) else {}

    def get_tags(self, obj: Product) -> list:
        return obj.suggested_tags if self._is_enriched(obj) else []

    def get_descriptions_i18n(self, obj: Product) -> dict:
        return obj.descriptions_i18n if self._is_enriched(obj) else {}

    def get_meta_title(self, obj: Product) -> str:
        if self._is_enriched(obj) and obj.meta_title:
            return obj.meta_title
        return obj.name

    def get_meta_description(self, obj: Product) -> str:
        if self._is_enriched(obj) and obj.meta_description:
...
class PublicStoreSerializer(serializers.ModelSerializer):
    """Buyer-facing storefront projection; excludes all KYC, payout and owner data."""
...
class ProductEnrichmentSerializer(serializers.ModelSerializer):
    """Read-only enrichment output for vendor review."""
...
class ApplyEnrichmentSerializer(serializers.Serializer):
    """Vendor selects which AI suggestions to apply to the live listing."""

    fields = serializers.ListField(
        child=serializers.ChoiceField(
            choices=["title", "description", "category", "seo"],
        ),
        allow_empty=False,
    )
