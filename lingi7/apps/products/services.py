...
class StoreService:
    ...
    def approve_store(store: Store, admin_user: Any) -> Store:
        ...
        AdminAuditLog.objects.create(
            ...
            target_content_type="products.store",
            target_object_id=str(store.pk),
            target_repr=f"Store({store.pk})",
...
class ProductService:
    ...
    def _transition(
        ...
        AdminAuditLog.objects.create(
            actor=actor,
            action_type=f"PRODUCT_{new_status}",
            target_content_type="products.product",
            target_object_id=str(product.pk),
            target_repr=f"Product({product.pk})",
            before_state={"status": before_state},
    ...
    def add_image(
        ...
        if getattr(settings, "CATALOG_AUTO_ENRICH_ON_IMAGE", False):
            from apps.products.tasks import enrich_product_task

            enrich_product_task.delay(product.pk)
    ...
    def reserve_stock(product: Product, quantity: int) -> InventoryRecord:
        ...
        Auto-creates InventoryRecord if it doesn't exist.
        ...
        record, created = InventoryRecord.objects.select_for_update().get_or_create(
            product=product,
            defaults={
                "quantity_available": 9999,  # Default unlimited inventory
                "track_inventory": False,    # Don't track by default
            }
        )
        ...
        record.quantity_available = F("quantity_available") - quantity
        record.quantity_held = F("quantity_held") + quantity
    ...
    def release_stock(product: Product, quantity: int) -> InventoryRecord:
        ...
        if record.quantity_held < quantity:
            raise InsufficientStockError(
                f"Cannot release {quantity} units for '{product.name}': "
                f"only {record.quantity_held} units held."
            )
        record.quantity_held = F("quantity_held") - quantity
        record.quantity_available = F("quantity_available") + quantity
    ...
    def deduct_stock(product: Product, quantity: int) -> InventoryRecord:
        ...
        if record.quantity_held < quantity:
            raise InsufficientStockError(
                f"Cannot deduct {quantity} units for '{product.name}': "
                f"only {record.quantity_held} units held."
            )
        record.quantity_held = F("quantity_held") - quantity
