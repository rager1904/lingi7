...
class KYCUploadSerializer(serializers.Serializer):
    ...
    def validate(self, data: dict) -> dict:
        ...
        if request and request.user.is_authenticated:
            ...
            for field in ("nrc_front_key", "nrc_back_key", "selfie_key"):
                key = data.get(field, "")
                if key:
                    data[field] = validate_kyc_storage_key(key, user_id)
        return data
...
class LingiTokenObtainPairSerializer(TokenObtainPairSerializer):
    ...
    def validate(self, attrs: dict) -> dict:
        ...
        if phone_field in attrs:
            attrs[phone_field] = UserManager._normalise_phone(attrs[phone_field])
