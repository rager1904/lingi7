...
class UserService:
    ...
    def kyc_submit(
        ...
        *,
        ...
        from apps.core.upload_validators import validate_kyc_storage_key

        uid = str(user.id)
        nrc_front_key = validate_kyc_storage_key(nrc_front_key, uid)
        nrc_back_key = validate_kyc_storage_key(nrc_back_key, uid)
        selfie_key = validate_kyc_storage_key(selfie_key, uid)
        ...
        logger.info(
            "kyc.submitted",
            extra={"user_id": str(user.id), "nrc": nrc_number[:3] + "****"},
        )
        return user
