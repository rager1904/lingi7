  ...
  return (
    <button
      ...
      style={{
        ...
        background: kind === "primary" ? "var(--color-accent)" : "var(--color-surface-base)",
...
export function Spinner({ description, ...props }: any) {
  return (
    <div {...props} style={{ display: "inline-flex", alignItems: "center", gap: 8 }}>
  ...
  return (
    <textarea
      ...
      style={{
        ...
        background: "var(--color-surface-base)",
        color: "var(--text-color-primary)",
        resize: resizeable === "manual" ? "vertical" : undefined,
        ...(props.style || {}),
      }}
    />
  );
...
export function FormField({ slotLabel, children }: any) {
  ...
  return (
    <label style={{ display: "block" }}>
      <span
        style={{ display: "block", marginBottom: 8, color: "var(--text-color-secondary)" }}
      >
        {slotLabel}
      </span>
    ...
    <div>
      <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
        {items?.map((item: any) => (
          <Button
            key={item.value}
            kind={item.value === active ? "primary" : "secondary"}
  ...
  return (
    <div>
      {items?.map((item: any) => (
        <details
          key={item.value}
          style={{ padding: 12, borderBottom: "1px solid var(--color-border-base)" }}
        >
          <summary>{item.slotTrigger}</summary>
          <div style={{ paddingTop: 12 }}>{item.slotContent}</div>
        </details>
      ))}
    </div>
  );
  ...
  return (
    <div style={{ display: "inline-flex", gap: 4 }}>
      {items?.map((item: any) => (
        <Button
          key={item.value}
          kind={item.value === value ? "primary" : "secondary"}
          size="small"
...
export function Modal({ open, onOpenChange, slotHeading, slotFooter, children, style }: any) {
  ...
  return (
    <div className="modal-backdrop" onClick={() => onOpenChange?.(false)}>
      <div className="modal-panel" style={style} onClick={(event) => event.stopPropagation()}>
        <Flex justify="between" align="center">
          <Text>{slotHeading}</Text>
          <Button size="small" onClick={() => onOpenChange?.(false)}>
            Close
          </Button>
        </Flex>
        <div style={{ marginTop: 16 }}>{children}</div>
        <div style={{ marginTop: 16 }}>{slotFooter}</div>
      </div>
    </div>
  );
}

export function Nebula({ variant = "ambient" }: { variant?: string }) {
  void variant;
