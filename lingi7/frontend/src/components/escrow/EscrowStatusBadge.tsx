...
export const EscrowStatusBadge: React.FC<Props> = ({ status, size = "md" }) => {
  const safeStatus = status in ESCROW_STATUS_LABEL ? status : "PENDING";
  const colour = ESCROW_STATUS_COLOUR[safeStatus as EscrowStatus];
  const label = ESCROW_STATUS_LABEL[safeStatus as EscrowStatus] ?? status;
