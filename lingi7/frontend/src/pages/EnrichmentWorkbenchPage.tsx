/**
 * EnrichmentWorkbenchPage — the AI product studio, now running as a first-class
 * page inside the Lingi7 Vite app. It shares the app's auth/axios stack and the
 * Django gateway at `/api/v1/products/enrichment-workbench/*`, so JWT auth,
 * vendor product ownership, persistence, and assistant indexing are unchanged.
 */

import React, { useEffect, useState, useRef } from "react";
import { Link } from "react-router-dom";
import { Stack, Nebula } from "../components/enrichment/ui-kit";
import { Header } from "../components/enrichment/Header";
import { ImageUploadCard } from "../components/enrichment/ImageUploadCard";
import { FieldsCard } from "../components/enrichment/FieldsCard";
import { AdvancedOptionsCard } from "../components/enrichment/AdvancedOptionsCard";
import { GeneratedVariationsSection } from "../components/enrichment/GeneratedVariationsSection";
import {
  ProductFields,
  AugmentedData,
  PolicyDocument,
...
import {
  ...
  prepareProductData,
  uploadPolicies,
  extractManualKnowledge,
} from "../api/enrichment";
import type { ProtocolSchemas } from "../api/enrichment";

export default function EnrichmentWorkbenchPage() {
    ...
    try {
      ...
      if (!silent) {
        setPolicyUploadError(
          error instanceof Error ? error.message : "Failed to load policy library"
        );
      }
    }
  };
  ...
  const handlePolicyFilesUpload = async (files: FileList | null) => {
    ...
    try {
      ...
      setPolicyUploadError(
        error instanceof Error ? error.message : "Failed to upload policy PDFs"
      );
    } finally {
  ...
  const handleClearPolicyLibrary = async () => {
    ...
    try {
      ...
      setPolicyUploadError(
        error instanceof Error ? error.message : "Failed to clear policy library"
      );
    } finally {
      setIsUploadingPolicies(false);
    }
  ...
  const handleManualFileUpload = async (file: File | undefined) => {
    ...
    try {
      ...
      setManualUploadError(
        error instanceof Error ? error.message : "Failed to extract manual knowledge"
      );
    } finally {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const variationParams = {
        ...
        tags: analyzeData.tags || [],
        colors: analyzeData.colors || [],
        enhancedProduct: (analyzeData as unknown as { enhanced_product?: unknown })
          .enhanced_product,
      };
        ...
        tasks.push(
          (async () => {
            try {
              ...
              if (!modelUrl) {
                setModel3DError(
                  "3D model generation completed but no model data was returned"
                );
              }
            } catch (error) {
  ...
  return (
    <div className="enrichment-shell">
      <div className="app-shell relative">
        ...
        <div className="relative" style={{ zIndex: 1 }}>
          <div className="mx-auto max-w-7xl px-4 pt-4">
            <Link
              to="/vendor"
              className="inline-flex items-center gap-1 text-sm font-semibold text-slate-300 transition hover:text-white"
            >
              ← Vendor dashboard
            </Link>
          </div>
              ...
              <Stack gap="8">
                ...
                <div className="studio-intro">
                  <div>
                    ...
                    <span>
                      Generate complete, policy-aware catalog content and premium visual
                      variations from one source image.
                    </span>
                  </div>
                ...
                <div>
                  <AdvancedOptionsCard
                    ...
                    onBrandInstructionsChange={(value) =>
                      setFields((prev) => ({ ...prev, brandInstructions: value }))
                    }
